...
export const ctrlCreateComment = async (req, res) => {
  ...
  try {
    ...
    console.log(error);
    res.status(500).json({ error: "No se pudo crear comentario" });
  }
};
...
export const ctrlListComments = async (req, res) => {
  ...
  if (!isCommentAuthor) {
    return res
      .status(403)
      .json({ error: "El usuario no es el creador del comentario" });
  }
...
export const ctrlGetCommentById = async (req, res) => {
  ...
  if (!isCommentAuthor) {
    return res
      .status(403)
      .json({ error: "El usuario no es el autor del comentario" });
  }
  ...
  try {
    ...
    res.status(500).json({ error: "No se puede traer el comentario" });
  }
};
...
export const ctrlUpdateComment = async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: "No se puede modificar el comentario" });
  }
};
...
export const ctrlDeleteComment = async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: "No se puede borrar comentario" });
  }
};
